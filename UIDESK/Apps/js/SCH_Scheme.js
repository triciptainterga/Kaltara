    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (i = 0; i < json.length; i++) {
                ...
                var dStart = new Date(json[i].PeriodeStart);
                var milisegundosStart = parseInt(json[i].PeriodeStart.replace("/Date(", "").replace(")/", ""));
                var newDateStart = new Date(milisegundosStart).toLocaleDateString("en-UE");
                var newTimeStart = new Date(milisegundosStart).toLocaleTimeString("en-UE");

                const date = new Date(newDateStart);
                const year = date.getFullYear();
                const month = String(date.getMonth() + 1).padStart(2, '0'); // Bulan mulai dari 0, jadi tambahkan 1
                const day = String(date.getDate()).padStart(2, '0');

                const formattedDate = `${year}-${month}-${day}`;

                var milisegundosEnd = parseInt(json[i].PeriodeEnd.replace("/Date(", "").replace(")/", ""));
                var newDateEnd = new Date(milisegundosEnd).toLocaleDateString("en-UE");
                var newTimeEnd = new Date(milisegundosEnd).toLocaleTimeString("en-UE");
                ...
                // var buttonAction = "<a onclick=Edit('" + json[i].ID + "','" + formattedDate + "','" + json[i].NameShifts + "')><i class='fas fa-edit'></i></a>  |  <i class='fas fa-trash-alt'></i>";
                var buttonAction = "<a onclick=\"Edit('" +
                    json[i].ID + "', '" +
                    formattedDate + "', '" +
                    json[i].NameShifts.replace(/'/g, "\\'") +
                    "','" +
                    json[i].Channel.replace(/'/g, "\\'") +
                    "')\"><i class='fas fa-edit'></i></a> | <i class='fas fa-trash-alt' onclick=\"Delete('" +
                    json[i].ID + "', '" +
                    json[i].Channel.replace(/'/g, "\\'") +
                    "')\"></i>";
                myTable.row.add([json[i].ID, json[i].NameShifts, json[i].Site, json[i].Channel, newDateStart, newDateEnd, newDate + ' ' + newTime, buttonAction]).draw(false);
            }
