    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (i = 0; i < json.length; i++) {
                ...
                myTable.row.add([json[i].ID, json[i].SiteName, json[i].Durasi, urlclick]).draw(false);
            }
        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                var form_data = JSON.stringify({
                    ID: $("#ContentPlaceHolder1_TrxID").val(), Durasi: $("#DurasiNya").val(), UserName: $("#hd_sessionLogin").val(), Action: 'UPDATE'
                });
                $.ajax({
